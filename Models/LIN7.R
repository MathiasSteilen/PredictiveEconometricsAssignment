...
print(paste("Starting LIN7 |", Sys.time()))

# Recipe: Data Preprocessing
lin_rec <- recipe(BMI ~ .,
                  data = dt_train) %>%
  step_novel(all_nominal_predictors()) %>%
  step_ns(Age, Height, deg_free = tune()) %>% 
  step_dummy(all_nominal_predictors()) %>% 
  step_interact(terms = ~ .*.) %>%
  step_rm(contains("BMI_"), skip = TRUE) %>% 
...
saveRDS(object = lin_final_fit, 
        file = paste0("LIN7_fit.rds"))

print(paste("LIN7 done |", Sys.time()))