...
print(paste("Starting LIN3 |", Sys.time()))

# Recipe: Data Preprocessing
lin_rec <- recipe(BMI ~ .,
                  data = dt_train) %>%
  step_novel(all_nominal_predictors()) %>%
  step_discretize(Age, num_breaks = 5, min_unique = 3) %>% 
  step_discretize(Height, num_breaks = 5, min_unique = 3) %>% 
...
saveRDS(object = lin_final_fit, 
        file = paste0("LIN3_fit.rds"))

print(paste("LIN3 done |", Sys.time()))